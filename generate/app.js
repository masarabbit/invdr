function init() {
  ...
  class mainCell extends Cell {
    constructor(props) {
      ...
      console.log(data.invaderCode)
      const invaderCode = data.invaderCode[props.index]
      super({
        properties: {
          x1: props.x1 || 0,
          y1: props.y1 || 0,
      ...
      if (
        ...
        (data.invaderCode.length
          ? data.invaderCode.length
          : this.body.invader.growthEnd)
      ) {
        this.split()
      }
  ...
  class Invader {
    constructor() {
      data.invaderCode = configs.map(c => {
